...
- Satır tutarları değiştirilebilir; hizmet veya ek ücret satırı eklenip çıkarılabilir.
...
- Altta o ayın hareketleri tarihe göre listelenir: ödemeler ve giderler. Satıra dokununca ilgili döküm veya gider açılır.
...
- Satırdaki "Hatırlat" butonu hazır bir WhatsApp mesajı açar. Mesaj şablonu E6'da tanımlanır.
...
| Hesaplanan değerler | Döküm durumu, kalan tutar, müşteri borcu ve toplam alacak saklanmaz; ödemelerden ve satırlardan hesaplanır. Müşteri listesi ve detayı yanıtlarında `balance` hazır hesaplanmış gelir (MUS-01, MUS-04). | KAS-01, KAS-04, KAS-08 |
...
| Dosya indirme | PDF yetkili bir uç noktadan gelir; `lib/api.ts`'e `downloadFile()` yardımcısı eklenir ki token ekleme ve 401/403 işleme tek yerde kalsın. Doğrudan `fetch` veya `FileSystem.downloadAsync` çağrısı feature kodunda olmaz. | KAS-06 |
...
| KAS-01, KAS-02 | OPR-03, KUR-08 | Ek ücret satırları |
...
| KAS-09 | E6 | Borç hatırlatma mesaj şablonu |
| KAS-08 | E8 | Veri dışa aktarma (kasa) |
